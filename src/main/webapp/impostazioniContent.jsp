...
<body>
					...
					<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
					                ...
					                <button id="addUserBtn" class="bg-red-500 hover:bg-red-600 text-white px-6 py-2 rounded-lg flex items-center gap-2 transition-colors">
 ...
 <!-- Users Table -->
        ...
        UserService utenti = new UserService();
        List<User> users = utenti.getAllUsers();
                                                ...
                                                <div class="flex items-center" 
						                    	   ...
						                    	   >
                                                    ...
                                                    <div class="flex-shrink-0 h-10 w-10">
                                                        <div class="w-10 h-10 rounded-full bg-[#d1d1d1] flex items-center justify-center">
									                        <i class="fas fa-user"></i>
									                    </div>
                                                    </div>
                                                    <div class="ml-4">
                                                        <div class="text-sm font-medium text-gray-900"><%= u.getUsername() %></div>
                                                        <div class="text-sm text-gray-500"><%= u.getMail() %> </div>
                                                    </div>
                                                </div>
                                            </td>
                                            <%
                                            String coloreRuolo="blue";
                                            if(u.getRuolo().equals("Developer")) 
                                            	coloreRuolo="purple";
                                            if(u.getRuolo().equals("Amministratore")) 
                                            	coloreRuolo="yellow";
                                            if(u.getRuolo().equals("Magazziniere")) 
                                            	coloreRuolo="orange";
                                            %>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-<%=coloreRuolo %>-100 text-<%=coloreRuolo %>-800">
                                                    <%= u.getRuolo() %> 
                                                </span>
                                            </td>
                                            <% 
                                            String coloreStato="green";
                                            if(u.getStato().equals("inattivo")) 
                                            	coloreStato="red";
                                            %>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-<%=coloreStato %>-100 text-<%=coloreStato %>-800">
                                                    <%= u.getStato() %> 
                                                </span>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div data-username="<%= u.getUsername() %>"
                                                	data-telefono="<%= u.getTelefono() %>"
						                    	     ...
						                    	     data-stato="<%= u.getStato() %>" class="userCard flex justify-end space-x-2">
						                    	     <%if(ruoloUtenteLoggato.equals("Amministratore") || session.getAttribute("userId").equals(u.getId())){ %>
                                                    <button class="editUser text-indigo-600 hover:text-indigo-900">
                                                        <i class="fas fa-user-edit"></i>
                                                    </button>
                                                    <%}else{ %>
                                                    <button class="viewUser text-indigo-600 hover:text-gray-900">
    ...
    %> 
            ...
            <div class="divide-y divide-gray-200">
                <div class="p-4 hover:bg-gray-50">
                    <div class="flex items-center">
                        <button id="btnGeneratePdf" class="fas fa-file-pdf text-red-600 bg-red-100 p-2 rounded-full mr-4">
                        </button>
                        
                        <div class="flex-1">
                            <p class="text-sm font-medium text-red-600">Scarica Etichette Qr</p>
                            <p class="text-xs text-gray-500">Scarica un file pdf con tutte le etichette degli articoli.</p>
                        </div>
                ...
                <div class="p-4 hover:bg-gray-50">
                    <div class="flex items-center">
                        <button id="btnGenerateExcel" class="fas fa-file-excel text-green-600 bg-green-100 p-2 rounded-full mr-4">
                        </button>
                        <div class="flex-1">
                            <p class="text-sm font-medium text-green-600">Esporta File Excel</p>
                            <p class="text-xs text-gray-500">Esporta un file excel con la lista completa degli articoli.</p>
                        </div>
                    </div>
                </div> 
                <%
               		if(session.getAttribute("ruolo").equals("Amministratore") || session.getAttribute("ruolo").equals("Magazziniere")){
                %>
               <div class="p-4 hover:bg-gray-50 rounded-xl border border-gray-200">
				  <form action="<%=request.getContextPath()%>/impostazioni"
				        method="post"
				...
				<div class="p-4 hover:bg-gray-50">
                    <div class="flex items-center">
                        <button id="openWipeModal" class="fas fa-trash text-red-800 bg-red-300 p-2 rounded-full mr-4">
                        </button>
                        <div class="flex-1">
                            <p class="text-sm font-medium text-red-800">Svuota Magazzino</p>
                            <p class="text-xs text-gray-500">Elimina definitivamente la lista degli articoli.</p>
                        </div>
                    </div>
                </div> 
				<%
				...
				<div id="wipeModal" class="fixed inset-0 bg-black/50 hidden z-50">
				  <div class="bg-white rounded-xl w-full max-w-lg mx-auto mt-24 p-6">
				    <h2 class="text-xl font-semibold text-red-700 mb-2"><i class="fas fa-warning"></i> Conferma distruttiva</h2>
				    <p class="text-sm text-gray-600 mb-4">
				      Operazione PERICOLOSA. Verranno azzerate tutte le giacenze.<br>
				      Step 1: inserisci la password del tuo account.
				    </p>
				
				    <form id="wipeStep1" method="post" action="<%=request.getContextPath()%>/impostazioni">
				      <input type="hidden" name="csrf" value="${sessionScope.csrfToken}">
				      <input type="hidden" name="step" value="preview">
				
				      <div class="mb-3">
				        <label class="block text-sm font-medium mb-1">Scrivi esattamente: <b>SVUOTA MAGAZZINO</b></label>
				        <input name="phrase" class="w-full border rounded-lg px-3 py-2" autocomplete=off placeholder="SVUOTA MAGAZZINO" required>
				      </div>
				
				      <div class="mb-3">
				        <label class="block text-sm font-medium mb-1">Password attuale</label>
				        <input name="passwordDelete" type="password" class="w-full border rounded-lg px-3 py-2" autocomplete=off required>
				      </div>
				
				      <div class="flex items-center gap-2 text-sm mb-4">
				        <input id="backupCheck" type="checkbox" class="h-4 w-4 border rounded" required>
				        <label for="backupCheck">Confermo di aver eseguito il backup oggi</label>
				      </div>
				
				      <div class="flex justify-end gap-2">
				        <button type="button" id="closeWipeModal" class="px-3 py-2 border rounded-lg">Annulla</button>
				    ...
				    <div id="previewBox" class="hidden mt-6 border-t pt-4">
				      <p class="text-sm text-gray-700 mb-2">Anteprima elementi interessati:</p>
				      <pre id="previewData" class="text-xs bg-gray-50 p-3 rounded"></pre>
				
				      <form id="wipeStep2" method="post" action="<%=request.getContextPath()%>/impostazioni">
				        <input type="hidden" name="csrf" value="${sessionScope.csrfToken}">
				        <input type="hidden" name="step" value="execute">
				        <input type="hidden" name="challengeId" id="challengeId">
				
				        <div class="mb-3">
				          <label class="block text-sm font-medium mb-1">Token inviato dal Dev</label>
				          <input name="otp" class="w-full border rounded-lg px-3 py-2" required>
				        </div>
				
				        <div class="flex justify-end gap-2">
				          <button type="button" id="cancelExec" class="px-3 py-2 border rounded-lg">Annulla</button>
				          <button class="px-3 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
            ...
            <!--  <div class="px-6 py-4 border-t border-gray-200 text-center">
                <a href="#" class="text-sm font-medium text-red-600 hover:text-red-800">Visualizza tutte le attività</a>
                                ...
                                <!-- Modal Backdrop -->
    <div id="modalBackdrop" class="fixed inset-0 bg-black bg-opacity-50 z-40 hidden animate-fade-in"></div>

    <!-- Modal Container -->
    <div id="profileModal" class="fixed inset-0 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden animate-slide-up">
            <!-- Modal Header -->
            <div class="bg-gradient-to-r from-red-600 to-gray-700 p-6 flex justify-between items-center">
                <div class="flex items-center space-x-3">
                    <i class="fas fa-user-cog text-white text-2xl"></i>
                    <h2 id="titolo" class="text-white text-xl font-bold">Modifica Utente</h2>
                </div>
            ...
            <!-- Modal Content -->
            <div class="custom-scrollbar overflow-y-auto p-6" style="max-height: calc(90vh - 120px)">
            	...
            	<input type="hidden" name="action" id="formAction" value="add">
                    ...
                    <!-- Ruolo -->
                    <div>
                        <label for="ruolo" class="block text-sm font-medium text-gray-700 mb-1">Ruolo</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <i class="fas fa-tag text-gray-400"></i>
                            </div>
                            <select id="role" name="ruolo"
						        class="pl-10 w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 shadow-sm" required>
                    ...
                    <!-- Phone -->
                    <div>
                        <label for="phone" class="block text-sm font-medium text-gray-700 mb-1">Numero di telefono</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            ...
            <div class="bg-gray-50 px-6 py-4 flex flex-col sm:flex-row justify-between items-center border-t border-gray-200">
                <button id="deleteAccountBtn" class="text-red-600 hover:text-red-800 font-medium text-sm mb-3 sm:mb-0">
                    <i class="fas fa-trash-alt mr-1"></i> Cancella Utente
                </button>
                <div class="space-x-3">
                    <button id="cancelBtn" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-100 transition-colors">
                        Annulla
                    </button>
                    <button id="saveBtn" type="submit" class="px-6 py-2 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 transition-colors shadow-md">
...
<div id="deleteModal" class="modal fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md fade-in">
            <div class="p-6">
                ...
                <p class="text-gray-600 text-center mb-6">Sei sicuro di voler eliminare questo utente? Questa azione non può essere annullata.</p>
