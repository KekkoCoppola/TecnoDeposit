...
public class FiltroServlet extends HttpServlet {
	  protected void doGet(HttpServletRequest request, HttpServletResponse response)
		    ...
		    try { size = Integer.parseInt(request.getParameter("size")); } catch (Exception ignore) {}
		    if (page < 1) page = 1;
		    if (size < 1) size = 24;
		    ...
		    try {
		      ...
		      if (stato != null && "Tutti".equals(stato)) stato = "";
		      if (marca != null && "Tutte".equals(marca)) marca = "";
		      ...
		      for (String stato1 : stati) {
		    	    ...
		    	    String nomeAttr = "count" + stato1
		    	            ...
		    	            .replace("è", "e");
		    	    if(stato1.equals("Non Riparato")) System.out.println(nomeAttr);
		      ...
		      for (Articolo a : pageItems) {
		        if (a.getImmagine() == null || a.getImmagine().isEmpty()) {
		          a.setImmagine(trovaImmagineArticolo(
		  ...
		  public static String trovaImmagineArticolo(String nome, String marca, String directoryPath, String baseUrl) {
		    String baseName = slugify(marca) + slugify(nome);
		    String[] estensioni = { ".png", ".jpg", ".jpeg", ".webp" };
		    File dir = new File(directoryPath);
		    if (!dir.exists()) return baseUrl + "/Icon.png";
		    File[] files = dir.listFiles();
		    if (files == null) return baseUrl + "/Icon.png";
		    for (String ext : estensioni) {
		      String target = (baseName + ext).toLowerCase();
		      for (File file : files) {
		        if (file.getName().toLowerCase().equals(target)) {
